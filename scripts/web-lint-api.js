    ...
    exec(
      ...
      (error, stdout, stderr) => {
        ...
        if (error) {
          ...
          resolve(new TestResult(command.split(" ")[0], "success", duration, stdout, stderr));
        }
      },
    );
  });
...
async function runSecurityCheck() {
  ...
  try {
    ...
    if (hardcodedEnvVars.length > 0 || hardcodedValues.length > 0) {
      ...
      output +=
        "⚠️ Güvenlik riski! Tüm API key'leri ve environment variable'ları .env dosyasına taşıyın.";
    } else {
      output =
    ...
    return new TestResult("Security Check", status, duration, output, "");
  } catch (error) {
    const endTime = Date.now();
    const duration = (endTime - startTime) / 1000;

    return new TestResult("Security Check", "error", duration, "", error.message);
  }
}
