...
const file = 'C:\\Users\\Melih\\Documents\\Projects\\akilhane\\src\\components\\enhanced-dashboard.tsx';
...
const replacements = [
  ...
  // Hook definition
  ['export default function EnhancedDashboard() {', 'export default function EnhancedDashboard() {\n  const t = useTranslations("Dashboard");'],

  // Toasts
  ['"Bu özellik sadece misafir kullanıcılar içindir"', 't("toasts.guestOnly")'],
  ['"Giriş yapmış kullanıcılar verilerini profil ayarlarından yönetebilir."', 't("toasts.guestOnlyDesc")'],
  ['"Veriler başarıyla dışa aktarıldı"', 't("toasts.exportSuccess")'],
  ['"Yedek dosyanız indirildi. Bu dosyayı güvenli bir yerde saklayın."', 't("toasts.exportSuccessDesc")'],
  ...
  ['"Demo veriler temizlendi. Misafir kullanıcı moduna geçildi. Sayfa yenileniyor..."', 't("toasts.demoInactiveDesc")'],
  ...
  ['<Button className="w-full">Giriş Yap</Button>', '<Button className="w-full">{t("login")}</Button>'],
  ...
  ['<strong className="text-gray-800 dark:text-white">Demo Modu Aktif!</strong>', '<strong className="text-gray-800 dark:text-white">{t("demoModeActive")}</strong>'],
  ['Bu veriler\\n                  örnek kullanım için hazırlanmış demo verileridir. Gerçek kullanım\\n                  deneyimini görmek için demo modunu kapatabilirsiniz.', '{t("demoModeDesc")}'],

  ['<strong className="text-gray-800 dark:text-white">\\n                    Misafir modunda kullanıyorsunuz.\\n                  </strong>', '<strong className="text-gray-800 dark:text-white">\\n                    {t("guestModeActive")}\\n                  </strong>'],
  ...
  ['result.type === "TopicExplainer" ? "Konu Anlatımı Okundu" : "Test Çözüldü"', 'result.type === "TopicExplainer" ? t("topicRead") : t("testSolved")'],
  ['doğru •{" "}', '{t("correct")} •{" "}'],
  ['>Okundu<', '>{t("read")}<'],

  ['Test çözerek veya konu çalışarak aktivitelerinizi görün ve gelişiminizi takip edin!', '{t("recentActivitiesEmptyDesc")}'],
