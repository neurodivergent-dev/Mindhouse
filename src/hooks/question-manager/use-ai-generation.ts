...
export const useAIGeneration = (
  ...
  const generateQuestions = useCallback(
    async (formData: {
      ...
      try {
        ...
        const { generateQuestions: aiGenerateQuestions } =
          await import("@/ai/flows/question-generator");
  ...
  const approveAIQuestions = useCallback(
    async (questionsToAdd: AIGeneratedQuestion[], subject: string) => {
      try {
        setIsCreating(true);
        ...
        for (const question of newQuestions) {
          if (isAuthenticated) {
            try {
              ...
              const subjectId =
                subjectObj?.id ||
                `subject_${subject.toLowerCase().replace(/[^a-z0-9]/g, "_")}_${Date.now()}`;
              ...
              const createQuestionWithTimeout = async () =>
                Promise.race([
              ...
              if (result) {
                ...
                const createdQuestion: Question = {
                  ...
                  text: result.text,
                  options: (typeof result.options === "string"
                    ? JSON.parse(result.options || "[]")
                    : result.options || []) as Answer[],
