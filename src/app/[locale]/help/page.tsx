...
type CategoryId =
  "all" | "general" | "ai" | "quiz" | "flashcard" | "premium" | "security" | "support";
  ...
  const categories = useMemo(
    () => [
      ...
      },
      {
        id: "ai" as const,
        name: t("categoryAi"),
        count: faqData.filter((f) => f.category === "ai").length,
      },
      {
        id: "quiz" as const,
        name: t("categoryQuiz"),
        count: faqData.filter((f) => f.category === "quiz").length,
      },
      {
        id: "flashcard" as const,
        name: t("categoryFlashcard"),
        count: faqData.filter((f) => f.category === "flashcard").length,
      },
      {
        id: "premium" as const,
        name: t("categoryPremium"),
        count: faqData.filter((f) => f.category === "premium").length,
      },
      {
        id: "security" as const,
        name: t("categorySecurity"),
        count: faqData.filter((f) => f.category === "security").length,
      },
      {
        id: "support" as const,
        name: t("categorySupport"),
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          <Card className="apple-glass-card border-0 shadow-xl rounded-3xl hover:scale-[1.02] transition-all duration-300">
            <CardContent className="p-8 text-center flex flex-col items-center justify-center">
              ...
              <h3 className="font-extrabold text-lg text-[#1d1d1f] dark:text-[#f5f5f7] mb-2 tracking-tight">
            ...
            <Card className="apple-glass-card border-0 shadow-2xl rounded-3xl overflow-hidden h-full">
              <CardHeader className="pt-8 px-8">
                <CardTitle className="flex items-center gap-3">
                  ...
                  <span className="text-2xl font-black tracking-tight text-[#1d1d1f] dark:text-[#f5f5f7]">
                    {t("faqTitle")}
                  </span>
                </CardTitle>
              ...
              <CardContent className="p-8">
                <div className="flex flex-wrap gap-2 mb-8">
                  {categories.map((category) => (
                    <Button
                      ...
                      className={`text-xs font-bold rounded-xl transition-all h-9 px-4 ${
                    ...
                    >
                      {category.name}{" "}
                      <span className="opacity-60 ml-1 font-semibold">({category.count})</span>
                    </Button>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  {filteredFAQ.map((faq, index) => (
                    <Card
                      key={faq.question}
                      className="border border-slate-100 dark:border-white/[0.03] bg-white/40 dark:bg-white/[0.02] shadow-sm rounded-2xl overflow-hidden transition-all duration-200"
                    >
                      <CardHeader
                      ...
                      >
                        <div className="flex items-center justify-between gap-4">
                          <CardTitle className="text-base font-bold text-[#1d1d1f] dark:text-[#f5f5f7]">
                            {faq.question}
                          </CardTitle>
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center transition-all bg-slate-100 dark:bg-white/5 ${expandedFAQ === index ? "rotate-180" : ""}`}
                          >
            ...
            <Card className="apple-glass-card border-0 shadow-2xl rounded-3xl overflow-hidden h-full">
              <CardHeader className="pt-8 px-8">
                <CardTitle className="flex items-center gap-3">
                  ...
                  <span className="text-xl font-black tracking-tight text-[#1d1d1f] dark:text-[#f5f5f7]">
                    {t("tutorialsTitle")}
                  </span>
                </CardTitle>
                  ...
                  >
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-sm text-[#1d1d1f] dark:text-[#f5f5f7] mb-1 truncate">
                          {tutorial.title}
                        </h4>
                        ...
                        <div className="flex items-center justify-between gap-2 mt-1">
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            className="text-xs h-7 px-3 font-extrabold rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 border-slate-200 dark:border-white/[0.08]"
                          >
          ...
          <CardContent className="p-8">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black tracking-tight text-[#1d1d1f] dark:text-[#f5f5f7] mb-1">
            ...
            <Card className="apple-glass-card border-0 shadow-xl rounded-3xl hover:scale-[1.02] transition-all duration-300">
              <CardContent className="p-6 text-center flex flex-col items-center justify-between h-full">
                <div className="flex flex-col items-center">
                  ...
                  <h3 className="font-extrabold text-base text-[#1d1d1f] dark:text-[#f5f5f7] mb-2 tracking-tight">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="w-full h-10 font-bold rounded-xl border-slate-200 dark:border-white/[0.08] hover:bg-slate-100 dark:hover:bg-white/5"
                >
